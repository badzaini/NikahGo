...
export default function ConsultationForm() {
  ...
  const handleConfirm = async () => {
    const insert = await createConsultation({ data });
      ...
      <Box sx={{ display: 'flex', justifyContent: 'center', bgcolor: 'white', borderRadius: '16px', marginTop: 2, boxShadow: 2, marginBottom: 4 }}>
        <Grid xs="auto">
          ...
          <FormControl sx={{ m: 1, minWidth: 120, maxWidth: 800, width: 800 }}>
            <TextField id="pengaduName" label="Nama Pengadu" variant="outlined" value={userName} onChange={(e) => setUserName(e.target.value)} />
          </FormControl>
          ...
          {/* IC/Passport Pengadu */}
          <FormControl sx={{ m: 1, minWidth: 120, maxWidth: 800, width: 800 }}>
            <TextField id="noICPengadu" label="No. IC / No. Passport  " variant="outlined" value={userIc} onChange={(e) => setUserIc(e.target.value)} />
          </FormControl>
          ...
          <LocalizationProvider dateAdapter={AdapterDayjs}>
            <FormControl sx={{ m: 1, minWidth: 120, maxWidth: 400, width: 400 }}>
              <TimePicker
                label={<Grid sx={{ display: 'flex' }}>Masa<Typography sx={{ color: 'red' }}>*</Typography></Grid>}
                value={time}
          ...
          <FormControl sx={{ m: 1, minWidth: 120, maxWidth: 800, width: 800, height: 100 }}>
            ...
            >
              ...
              <MenuItem value={"Abu bin Ali"}>Lelaki - Abu bin Ali</MenuItem>
            </Select>
          </FormControl>
          ...
          <Grid container spacing={2} sx={{ display: 'flex', textAlign: 'center', marginBottom: 4 }}>
            ...
            <Grid md={4}>
              <Button size="large" variant="contained" onClick={handleConfirm} sx={{ bgcolor: "#49516F" }}>Hantar</Button>
            </Grid>
            <Grid md={4}>
